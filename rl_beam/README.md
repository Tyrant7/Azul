...
`rl_beam` exposes a trained actor through the interface crate's UAI runtime.
The actor generates candidate moves, while the matching trained critic
evaluates search states from the root player's perspective. The critic is a
scalar MSE value network.
...
The default critic checkpoint is `checkpoints/azul_critic.ot`. The default beam
width is `4` and depth is `8` plies. The actor and critic should come from the
same training run.
