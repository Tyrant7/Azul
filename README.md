...
The interface executable is currently a development harness. It parses CLI configuration, can spawn configured child processes, creates a local two-player game, prints the state, and accepts human-entered moves. Engine stdin/stdout wiring, command dispatch, tournaments, timing, recovery, and most UAI behavior remain to be implemented.
